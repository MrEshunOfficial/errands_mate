...
export default function ProvidersPage() {
  ...
  const { preference, addFavoriteProvider, removeFavoriteProvider } =
    useClientPreference();

  const favoriteProviderIds = useMemo<string[]>(
    () =>
      (preference?.favoriteProviders ?? []).map((entry) =>
  ...
  // ── Scroll detection — tracks the inner scrollable main element ──────────
  useEffect(() => {
  ...
  return (
    <main ref={mainRef} className="h-full overflow-y-auto hide-scrollbar bg-stone-50 dark:bg-stone-950">
      {/* ── Compact Sticky Bar ───────────────────────────────────────────── */}
      <div
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-3 sm:gap-4">
                  {nearbyProviders.map((provider, i) => (
                    <ProviderCard
                      ...
                      provider={provider}
                      isNearby={i < 3}
                      isFavorite={favoriteProviderIds.includes(
                        String(provider._id),
                      )}
                      onToggleFavorite={isCustomer ? handleToggleFavorite : undefined}
                    />
                  ))}
                </div>
              </section>
