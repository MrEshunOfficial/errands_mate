...
// Provider:  Role → Contact → Logo → Ghana Card → Done
// Customer:  Role → Contact → Photo → Done

function getStepLabels(role: Role | null): readonly string[] {
  if (role === "provider")
    return ["Role", "Contact", "Logo", "Ghana Card", "Finish"] as const;
  return ["Role", "Contact", "Photo", "Finish"] as const;
}
...
function OnboardingProgress({
  ...
  const pct = Math.round((clamped / maxStep) * 100);
...
function StepRole({ role, onSelect, onNext }: StepRoleProps) {
  return (
...
function StepPhoto({
  ...
  return (
    <div className="p-6 flex flex-col gap-5">
      <div>
        <h2 className="text-lg font-semibold tracking-tight">
          {isProvider ? "Upload your business logo" : "Add a profile photo"}
        </h2>
        <p className="text-sm text-muted-foreground mt-1">
          {isProvider
            ? "Your logo will appear on your service listings. You can update it anytime."
            : "Helps people recognise you. You can update this anytime."}
        </p>
      </div>

      {uploaded ? (
        <div
          className="flex items-center gap-3 px-4 py-3 rounded-xl border border-emerald-200 bg-emerald-50 text-emerald-800"
          role="status">
        ...
        <ProfilePictureUploader onUploadSuccess={onUploaded} />
      )}
...
// ─── Step 3 (providers only): Ghana Card Details ───────────────────────────────

type GhanaCardPhase = "details" | "upload" | "done";

interface StepGhanaCardProps {
  profileHook: ReturnType<typeof useProfile>;
...
function StepGhanaCard({
  ...
  const [phase, setPhase] = useState<GhanaCardPhase>("details");
      ...
      <div className="flex flex-col items-center text-center gap-2">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1 max-w-xs mx-auto">
            Submit your Ghana Card number and photos to build client trust,
            receive payments, and earn your Verified badge.
          </p>
        </div>
      </div>
        ...
        <>
          <div className="space-y-1.5">
            ...
            <p className="text-xs text-muted-foreground">
              Found on the front of your Ghana Card — format: GHA-XXXXXXXXX-X
            </p>
            <Input
              id="card-number"
              value={cardNumber}
          ...
          <div className="flex flex-col gap-2">
            <p className="text-xs text-muted-foreground bg-muted/50 rounded-lg p-3">
              Next, you&apos;ll upload clear photos of both the{" "}
              <strong>front</strong> (photo &amp; name side) and{" "}
              <strong>back</strong> (barcode &amp; address side) of your Ghana
              Card.
            </p>
            <Button
              ...
              {pL.updatingIdDetails
                ? "Saving…"
                : "Continue to Upload Ghana Card Photos"}
            </Button>
            <Button
              variant="ghost"
              onClick={onSkip}
      ...
      {phase === "upload" && (
        <>
          <div className="space-y-1">
            <p className="text-sm font-medium">Ghana Card Photos</p>
            <p className="text-xs text-muted-foreground">
              Upload clear, well-lit photos of both sides. All text and the
              photo on the card must be fully visible.
            </p>
          </div>
          ...
          <Button
            ...
            onClick={onDone}
            className="w-full text-muted-foreground">
            Proceed without photos (limited access)
          </Button>
        </>
      )}
          ...
          <Alert className="border-emerald-200 bg-emerald-50 text-emerald-800">
            ...
            <AlertDescription>
              {uploadedFiles.length > 0
                ? `${uploadedFiles.length} Ghana Card photo${uploadedFiles.length !== 1 ? "s" : ""} uploaded. Your verification is under review.`
                : "Ghana Card photos uploaded. Your verification is under review."}
            </AlertDescription>
          </Alert>
...
> = {
  provider: {
    ...
    sub: "Complete your Ghana Card verification to set up your business profile and start accepting clients.",
    cta: "Set Up Business Profile",
  },
  customer: {
    emoji: "👋",
    title: "Welcome aboard!",
    sub: "take a glance at your dashboard",
    cta: "Go to dashboard",
  },
};
...
// 1 → Contact
// 2 → Photo / Logo
// 3 → Ghana Card (provider only) | Final (customer)
// 4 → Final (provider only)
...
export default function ProfileOnboarding({
  ...
  const handleContactNext = async () => {
    ...
    if (!result) return; // API error — stay on step 1
  ...
  // ── Ghana Card handlers ───────────────────────────────────────────────────
  const handleGhanaCardDone = () => {
    setFinalVariant("provider");
    go(4);
  };
  const handleGhanaCardSkip = () => {
    setFinalVariant("provider");
    go(4);
  };
  ...
  const handleComplete = () => {
    onComplete?.(role!);
    router.push(role === "provider" ? "/profile" : "/dashboard");
  };

  // Progress step: map logical step to display step index in the label array
  // Step 0 (Role)=0, Step 1 (Contact)=1, Step 2 (Photo)=2, Step 3 (GhanaCard/Final)=3, Step 4 (Final provider)=4
  const progressStep = step;
  const showProgress = step <= (role === "provider" ? 3 : 2);

  const renderStep = () => {
    // Step 0: Role selection
    if (step === 0)
      return <StepRole role={role} onSelect={setRole} onNext={() => go(1)} />;

    // Step 1: Contact details + profile creation
    if (step === 1)
      return (
        <StepContact
    ...
    // Step 2: Photo (customer) or Logo (provider)
    if (step === 2)
      return (
        <StepPhoto
          role={role!}
    ...
    // Step 3: Ghana Card (provider) or Final (customer)
    if (step === 3) {
      if (role === "provider") {
        return (
          <StepGhanaCard
            ...
            onDone={handleGhanaCardDone}
            onSkip={handleGhanaCardSkip}
          />
        );
      }
      // Customer final
    ...
    // Step 4: Final (provider only)
    return (
      <FinalScreen
        variant={finalVariant ?? "provider"}
        onComplete={handleComplete}
      />
    );
  };
  ...
  return (
    <Card className="w-full max-w-110 overflow-hidden shadow-lg">
      {showProgress && <OnboardingProgress step={progressStep} role={role} />}
