...
import { RefreshCw, AlertTriangle, Ban, GitBranch } from "lucide-react";
    ...
    <AlertDialog open={open} onOpenChange={onOpenChange}>
      <AlertDialogContent className="max-w-md">
        ...
        <div className="flex justify-center pb-1 pt-2">
          <div className="flex size-12 items-center justify-center rounded-full border border-amber-200 bg-amber-50 dark:border-amber-800 dark:bg-amber-950/40">
          ...
          <AlertDialogDescription asChild>
            <div className="space-y-4 pt-1 text-left">
              <p className="text-sm text-muted-foreground">
                ...
                . To keep the category tree consistent, a parent category cannot
                be deleted while it still has active subcategories beneath it.
              </p>

              <div className="rounded-lg border bg-muted/40 p-3 text-[13px] text-muted-foreground">
                ...
                <ul className="space-y-1.5">
                  <li className="flex items-start gap-2">
                    ...
                    <span>
                      Edit each subcategory and reassign it to a different
                      parent.
                    </span>
                  </li>
                  <li className="flex items-start gap-2">
...
export default function CategoryListPanel() {
  ...
  const openDeleteAlert = useCallback(
    (id: string, permanent: boolean) => {
      const activeChildren = categoriesArray.filter(
        (c) => c.parentCategoryId === id && !c.isDeleted,
      );
      if (activeChildren.length > 0 && !permanent) {
        const category = categoriesArray.find((c) => c._id === id);
        setBlockedDeleteInfo({
          categoryName: category?.catName ?? "this category",
  ...
  return (
    <div className="flex h-full w-full flex-col gap-3">
      ...
      {/* Blocked delete — category has active children */}
      <CannotDeleteDialog
        open={blockedDeleteOpen}
        onOpenChange={setBlockedDeleteOpen}
        categoryName={blockedDeleteInfo?.categoryName ?? ""}
        childCount={blockedDeleteInfo?.childCount ?? 0}
      />
