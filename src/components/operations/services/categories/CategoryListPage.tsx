...
import React, { useState, useMemo, useCallback, useEffect, useRef, JSX } from "react";
...
function CategoryCard({
  ...
  const coverObj =
    category.catCoverId && typeof category.catCoverId === "object"
      ? category.catCoverId
      : null;
  const hasCover = Boolean(coverObj?.url);
      ...
      <div className="relative h-32 bg-linear-to-br from-amber-50 to-orange-50 dark:from-stone-800 dark:to-stone-700 overflow-hidden shrink-0">
        ...
        {onToggleFavorite && (
          <button
            ...
            onClick={(e) => {
              e.stopPropagation();
              onToggleFavorite(category._id.toString());
            }}
            className={`absolute top-3 right-3 w-7 h-7 rounded-full border flex items-center justify-center transition-all shadow-sm ${
              isFavorite
...
export default function CategoryListPage(): JSX.Element {
  ...
  const { preference, addFavoriteCategory, removeFavoriteCategory } =
    useClientPreference();
  ...
  const handleToggleFavorite = useCallback(
    (id: string) => {
      const action = favoriteCategoryIds.includes(id)
        ? removeFavoriteCategory(id)
        : addFavoriteCategory(id);
      action.then((ok) => {
        if (!ok) toast.error("Could not update saved categories. Please try again.");
      });
    },
    [favoriteCategoryIds, addFavoriteCategory, removeFavoriteCategory],
  );
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                    {paginatedCategories.map((cat) => (
                      <CategoryCard
                        ...
                        category={cat}
                        onView={handleView}
                        isFavorite={favoriteCategoryIds.includes(
                          cat._id.toString(),
                        )}
