...
export default function UserProfileNav({
  ...
  // ── Provider profile (for rating stats) ──────────────────────────────────
  ...
  const handleMobileNavClick = useCallback(
    (link: NavigationLink) => {
      handleNavClick(link);
      if (!link.children?.length) onMobileClose?.();
    },
    [handleNavClick, onMobileClose],
  );
  ...
  return (
    <TooltipProvider delayDuration={300}>
                    ...
                    <div className="flex items-center gap-6 mt-3 pt-3 border-t border-white/15">
                      {isProvider ? (
                        <>
                          <div className="flex flex-col gap-0.5">
                            <div className="flex items-center gap-1 leading-none">
                              ...
                              <span className="text-base font-bold text-white tabular-nums leading-none">
                                {(providerProfile?.ratingStats?.count ?? 0) > 0
                                  ? providerProfile!.ratingStats!.average.toFixed(1)
                                  : "—"}
                              </span>
                            </div>
                          ...
                          <div className="flex flex-col gap-0.5">
                            <span className="text-base font-bold text-white tabular-nums leading-none">
                              {providerProfile?.ratingStats?.count ?? 0}
                            </span>
                            <span className="text-[9px] font-medium text-white/50 uppercase tracking-widest whitespace-nowrap">
                              Reviews
                            </span>
      ...
      {/* Mobile Sheet — renders via portal at document body */}
      <Sheet
        open={isMobileOpen ?? false}
        onOpenChange={(open) => {
        ...
        <SheetContent side="left" className="p-0 w-80 flex flex-col">
          ...
          <div className="flex items-center gap-3 px-4 py-4 border-b border-gray-200 dark:border-gray-800 shrink-0">
            {isLoading ? (
              <div className="w-10 h-10 rounded-full bg-slate-100 dark:bg-gray-800 animate-pulse shrink-0" />
            ) : (
              <AvatarChanger
                src={avatarUrl}
                name={displayName}
            ...
            <div className="flex-1 min-w-0">
              {isLoading ? (
                <div className="space-y-1.5">
                  ...
                  <div className="h-3 w-32 bg-slate-100 dark:bg-gray-800 rounded animate-pulse" />
                </div>
              ) : (
                <>
          ...
          {/* Nav tree */}
          <ScrollArea className="flex-1 overflow-y-auto px-3 py-3 hide-scrollbar">
            <NavTree
              links={navigationLinks}
              isCollapsed={false}
              isLoading={isLoading}
