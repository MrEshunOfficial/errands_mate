...
export function IdImageUploader({
  ...
  const handleUpload = async () => {
    ...
    // Mark all pending slots as uploading
    setSlots((s) => {
      const next = { ...s };
    ...
    // KEY FIX: `results !== null` means the API call succeeded (even if the
    // response body yielded no parseable file records). `null` means the
    // call threw — the hook has already set errors.mutation in that case.
    const uploadSucceeded = results !== null;

    setSlots((s) => {
      const next = { ...s };
      pendingKeys.forEach((k, i) => {
        if (!uploadSucceeded) {
          // Actual API error — let the user retry
          next[k] = { ...next[k], status: "error" };
        } else {
          // Success — mark done regardless of whether we got a file record back
          const uploadedFile = results[i] ?? undefined;
          next[k] = {
            ...next[k],
            status: "done",
    ...
    if (uploadSucceeded) {
      // Use actual file count if available, else fall back to how many we sent
      const count = results.length > 0 ? results.length : pendingKeys.length;
      const msg = `${count} image${count > 1 ? "s" : ""} uploaded successfully`;
      setSuccessMsg(msg);
      toast.success("ID images uploaded successfully");
      // Pass the file records (possibly empty array) to parent
      onUploadSuccess?.(results);
    } else {
      // errors.mutation from the hook already holds the server message;
      // also show a toast so it's immediately visible
      toast.error("Failed to upload ID images. Please try again.");
    }
  };
    ...
    <div className={`max-w-130 ${className}`}>
      ...
      <div className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-700 rounded-2xl overflow-hidden shadow-sm">
        ...
        <div className="px-6 pb-6 pt-5 flex flex-col gap-4">
          ...
          {/* Error banner — shown when the hook sets errors.mutation */}
          {errors.mutation && (
            <div
