...
//
// Gallery list endpoints return { fileId, url, thumbnailUrl, uploadedAt, metadata }
// rather than a full IFile document. Normalise to IFile so consumers don't need
...
function normaliseGalleryItem(raw: RawGalleryItem): IFile {
  const id = (raw._id ?? raw.fileId) as string;
  const fileName =
    raw.fileName ??
    raw.metadata?.publicId?.split("/").pop() ??
    id;
  return {
    _id: id,
    url: raw.url,
...
interface UseProviderGalleryActions {
  ...
  // Cloudinary — reads (auth)
  fetchMyGallery: () => Promise<IFile[]>;
...
export function useProviderGallery(
  ...
  // ── Auth-gated fetch (owner) ─────────────────────────────────────────────────

  const fetchMyGallery = useCallback(async (): Promise<IFile[]> => {
    setIsLoading(true);
    setError(null);
    try {
      const result = await providerGalleryAPI.getMyGallery();
      const files = (result.files as unknown as RawGalleryItem[]).map(
  ...
  const fetchPublicGallery = useCallback(
    async (id: string): Promise<IFile[]> => {
      ...
      try {
        ...
        if (id === providerId) setGalleryImages(files);
        return files;
      } catch (err) {
        captureError(err, "Failed to fetch gallery");
        return [];
