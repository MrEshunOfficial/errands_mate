...
class ProviderGalleryAPI extends APIClient {
  ...
  /**
   * Returns all gallery images for the currently authenticated provider.
   * Auth-gated — token identifies the owner, no providerId required.
   */
  async getMyGallery(): Promise<PublicProviderGalleryResponse> {
    return this.get<PublicProviderGalleryResponse>(
      `${this.base}/gallery/cloudinary`,
    );
  }
