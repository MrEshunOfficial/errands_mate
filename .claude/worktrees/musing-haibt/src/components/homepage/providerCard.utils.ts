...
import { ProviderMatchResult } from "@/types/task/task.types";
...
export async function fetchProviderProfile(
  ...
  try {
    ...
    const data: ProviderProfileResponse = await res.json();
    if (!data.success || !data.providerProfile) return null;

    const p = data.providerProfile;
    return {
      _id: p._id,
      businessName: p.businessName,
      providerContactInfo: p.providerContactInfo
        ? {
            primaryContact: p.providerContactInfo.primaryContact,
            businessEmail: p.providerContactInfo.businessEmail,
          }
        : undefined,
