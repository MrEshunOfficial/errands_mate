...
const STATUS_CARDS = [
  ...
  {
    label: "Confirmed",
    getValue: (s: BookingStats) => s.byStatus?.[BookingStatus.CONFIRMED],
    icon: CheckCircle2,
    accent: "bg-blue-100 text-blue-600 dark:bg-blue-950 dark:text-blue-300",
  },
  {
    label: "In Progress",
    getValue: (s: BookingStats) => s.byStatus?.[BookingStatus.IN_PROGRESS],
    icon: Play,
    accent: "bg-amber-100 text-amber-600 dark:bg-amber-950 dark:text-amber-300",
  },
  {
    label: "Awaiting Review",
    getValue: (s: BookingStats) => s.byStatus?.[BookingStatus.AWAITING_VALIDATION],
    icon: Eye,
    accent: "bg-violet-100 text-violet-600 dark:bg-violet-950 dark:text-violet-300",
  },
  {
    label: "Disputed",
    getValue: (s: BookingStats) => s.byStatus?.[BookingStatus.DISPUTED],
    icon: AlertTriangle,
    accent: "bg-red-100 text-red-600 dark:bg-red-950 dark:text-red-300",
  },
  {
    label: "Rebuttal",
    getValue: (s: BookingStats) => s.byStatus?.[BookingStatus.REBUTTAL_SUBMITTED],
    icon: ClipboardList,
...
const PAYMENT_CARDS = [
  {
    label: "Validated",
    getValue: (s: BookingStats) => s.byStatus?.[BookingStatus.VALIDATED],
    icon: CheckCheck,
    accent: "bg-emerald-100 text-emerald-600 dark:bg-emerald-950 dark:text-emerald-300",
  },
  {
    label: "Completed",
    getValue: (s: BookingStats) => s.byStatus?.[BookingStatus.COMPLETED],
    icon: CheckCircle2,
    accent: "bg-teal-100 text-teal-600 dark:bg-teal-950 dark:text-teal-300",
  },
  {
    label: "Cancelled",
    getValue: (s: BookingStats) => s.byStatus?.[BookingStatus.CANCELLED],
    icon: XCircle,
    accent: "bg-zinc-100 text-zinc-500 dark:bg-zinc-800 dark:text-zinc-400",
  },
  {
    label: "Paid",
    getValue: (s: BookingStats) => s.byPaymentStatus?.[PaymentStatus.PAID],
    icon: CheckCircle2,
    accent: "bg-emerald-100 text-emerald-600 dark:bg-emerald-950 dark:text-emerald-300",
  },
  {
    label: "Pending Payment",
    getValue: (s: BookingStats) => s.byPaymentStatus?.[PaymentStatus.PENDING],
    icon: Eye,
    accent: "bg-yellow-100 text-yellow-600 dark:bg-yellow-950 dark:text-yellow-300",
  },
  {
    label: "Failed Payments",
    getValue: (s: BookingStats) => s.byPaymentStatus?.[PaymentStatus.FAILED],
    icon: Ban,
    accent: "bg-red-100 text-red-600 dark:bg-red-950 dark:text-red-300",
  },
] as const;
...
function HealthMetrics({ stats }: HealthMetricProps) {
  ...
  const completionRate = Math.round(
    ((stats.byStatus?.[BookingStatus.COMPLETED] ?? 0) / total) * 100,
  );
  const disputeRate = Math.round(
    (((stats.byStatus?.[BookingStatus.DISPUTED] ?? 0) +
      (stats.byStatus?.[BookingStatus.REBUTTAL_SUBMITTED] ?? 0)) /
      total) *
      100,
  );
  const paymentSuccessRate = Math.round(
    ((stats.byPaymentStatus?.[PaymentStatus.PAID] ?? 0) / total) * 100,
  );

  const activeCount =
    (stats.byStatus?.[BookingStatus.CONFIRMED] ?? 0) +
    (stats.byStatus?.[BookingStatus.IN_PROGRESS] ?? 0) +
    (stats.byStatus?.[BookingStatus.AWAITING_VALIDATION] ?? 0);
...
function AttentionBanner({ stats }: { stats: BookingStats }) {
  const disputed = (stats.byStatus?.[BookingStatus.DISPUTED] ?? 0) +
    (stats.byStatus?.[BookingStatus.REBUTTAL_SUBMITTED] ?? 0);
  const pendingValidation = stats.byStatus?.[BookingStatus.AWAITING_VALIDATION] ?? 0;
  const failedPayments = stats.byPaymentStatus?.[PaymentStatus.FAILED] ?? 0;

  const issues: string[] = [];
  if (disputed > 0) issues.push(`${disputed} dispute${disputed !== 1 ? "s" : ""} need resolution`);
  if (pendingValidation > 0) issues.push(`${pendingValidation} booking${pendingValidation !== 1 ? "s" : ""} awaiting validation`);
  if (failedPayments > 0) issues.push(`${failedPayments} failed payment${failedPayments !== 1 ? "s" : ""}`);
