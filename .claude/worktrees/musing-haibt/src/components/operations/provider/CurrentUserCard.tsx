...
// import { useMyClientProfile } from "@/hooks/profiles/useClientProfile";
...
//   const { data: profile, loading } = useMyClientProfile();

//   if (loading) return <CardSkeleton />;
//   if (!profile) return null;

//   // `contactInfo.mainContact` is the primary phone number per the shared contactInfo shape
//   const phone = profile.contactInfo?.mainContact;
//   const email = profile.contactInfo?.businessEmail;

//   // Resolve the default saved address from the Location[] list
//   const locations = profile.savedLocations ?? [];
//   const defaultLocation =
//     locations.find((l) => l.isDefault) ?? locations[0] ?? null;
//   const addressLabel =
//     defaultLocation?.customLabel ?? defaultLocation?.label ?? "Default address";
...
//       <div className="p-4 space-y-3">
//         <LocationBlock
//           loc={defaultLocation?.address ?? null}
//           accentColor="emerald"
//           label={addressLabel}
//         />
...
export function CurrentUserCard() {
  ...
  if (role === UserRole.CUSTOMER) return <div>client panel</div>;
